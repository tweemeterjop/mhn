...
wget http://downloads.sourceforge.net/project/amunhoney/amun/amun-v0.2.2/amun-v0.2.2.tar.gz
tar --strip-components=1 --group=root --owner=root -zxvf amun-v0.2.2.tar.gz

mkdir -p $AMUN_HOME/log_modules/log-hpfeeds
cat > $AMUN_HOME/log_modules/log-hpfeeds/log_hpfeeds.py <<EOF
"""
...
cat > $AMUN_HOME/conf/log-hpfeeds.conf <<EOF
[log-hpfeeds]
server: $HPF_HOST
port: $HPF_PORT
...
#enable hpfeeds logging
sed -i '/log_modules:/a \\tlog-hpfeeds' $AMUN_HOME/conf/amun.conf
...
[program:amun]
command=python amun_server.py
directory=$AMUN_HOME
stdout_logfile=/var/log/amun.out
stderr_logfile=/var/log/amun.err
autostart=true
autorestart=true
